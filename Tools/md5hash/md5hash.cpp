...
static inline uint4 rotate_left(uint4 x, uint4 n) { return (x << n) | (x >> (32-n)); }
...
static void transform(uint4 state[4], uint1 block[64]) {
...
static void setmagicnum(uint4 state[4], int keyindex) {
    if (keyindex < 0 || keyindex >= 256) return;
...
static void md5_update(uint4 state[4], uint4 count[2], uint1 buffer[64], int* buffer_index,
                        const uint1* data, int datalen)
{
    uint4 oldcount = count[0];
    count[0] += (uint4)(datalen << 3);
    if (count[0] < oldcount) count[1]++;
    count[1] += (uint4)(datalen >> 29);

    int space = 64 - *buffer_index;
    int pos = 0;
    if (datalen >= space) {
        memcpy(buffer + *buffer_index, data, space);
        transform(state, buffer);
        pos = space;
        while (pos + 64 <= datalen) {
            transform(state, (uint1*)(data + pos));
            pos += 64;
        }
        memcpy(buffer, data + pos, datalen - pos);
        *buffer_index = datalen - pos;
    } else {
        memcpy(buffer + *buffer_index, data, datalen);
        *buffer_index += datalen;
    }
}

static void md5_encode_string(const char* input, int keyindex, char* output) {
    uint4 state[4];
    uint4 count[2] = {0, 0};
    uint1 buffer[64] = {0};
    int buffer_index = 0;
    ...
    setmagicnum(state, keyindex);

    md5_update(state, count, buffer, &buffer_index, (const uint1*)input, (int)strlen(input));
    ...
    md5_update(state, count, buffer, &buffer_index, PADDING, padLen);
    md5_update(state, count, buffer, &buffer_index, bits, 8);

    encode(digest, state, 16);
    for (int i = 0; i < 16; i++)
        wsprintf(output + i * 2, "%02X", digest[i]);
    output[32] = '\0';
}

static DWORD MakeAccountKey(const char* account) {
...
int main(int argc, char* argv[]) {
    if (argc < 3) {
        printf("Usage: md5hash.exe <account> <password>\n");
        return 1;
    }
    DWORD key = MakeAccountKey(argv[1]);
    char hex[33] = {0};
    md5_encode_string(argv[2], (int)key, hex);
    printf("%s", hex);
    return 0;
}
