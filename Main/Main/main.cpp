...
void __declspec(naked) ChaosBoxDurationItens()
{
	...
	if (ChaosBoxItemCode == GET_ITEM(12, 15) || ChaosBoxItemCode == GET_ITEM(14, 13) ||
		...
		(ChaosBoxItemCode >= GET_ITEM(14,474) && ChaosBoxItemCode <= GET_ITEM(14, 511) ||
			ChaosBoxItemCode == GET_ITEM(14, 345))
		)
	{
		_asm
		{			
...
DWORD SoulSwordJump1 = 0x686F21; //se não é
DWORD SoulSwordJump2 = 0x686D61; //se é Holy
DWORD SoulSwordJump3 = 0x68D22E; //se é Soul
...
void __declspec(naked) SoulSwordEffectAndTransparency()
{
	...
	//Dark Angel Effect
		...
		|| SoulSwordItemCode == GET_ITEM_MODEL(4, 56) //FE silver herat bow
		...
		|| SoulSwordItemCode == GET_ITEM_MODEL(0, 81) //DK silver sw
		|| SoulSwordItemCode == GET_ITEM_MODEL(0, 82) //MG silver sw
...
{
	gInterface.DrawFormat(eGold, 498, 78, 90, eTxtAlign_left, 0, "Seu Desconto Atual: %d%%", Discount);
...
char* GameShopValorFinal = (char*)malloc(16);
char* ValueFromString;
char* ValueFromString2;
int coinValue;
char* fakeOutput;
...
void MakeString(int coinValue, char* coinName)
{
	if ((int)((float)coinValue * (1.0f - ((float)Discount / 100.0f))) > 0)
	{
		sprintf(GameShopValorFinal, "De %d por %d %s", coinValue, (int)((float)coinValue * (1.0f - ((float)Discount / 100.0f))), coinName);
	}
	else
	{
		sprintf(GameShopValorFinal, "%d %s", coinValue, coinName);
	}
}
...
void __declspec(naked) GameShopValorFinalCompra1()
{
	...
	ValueFromString2 = strtok(NULL, " ");
	
	MakeString(coinValue, ValueFromString2);

	if (Discount > 0)
	{
		_asm
		{
			push GameShopValorFinal
		}
	}
	else
	{
...
DWORD GameShopValorFinalJump2 = 0x9ACA74;

void __declspec(naked) GameShopValorFinalCompra2()
{
	_asm
	{
		push 0x00
...
void EntryProc()
{
	...
	//SetByte(0x629706, 0x05); //Fumaça
	//SetByte(0x65A31B, 0x05); //Brilho do chão
	...
	//GameShop String Preço
	MemorySet(0x9ADE65, 0x90, 25);
	SetCompleteHook(0xE9, 0x9ADE65, &GameShopValorFinalCompra1);

	MemorySet(0x9ACA4A, 0x90, 25);
	SetCompleteHook(0xE9, 0x9ACA4A, &GameShopValorFinalCompra2);
	...
	SetCompleteHook(0xE9, 0xA8F63D, &MGDamageMultiplier); //FOI PRO S16

	//CustomJewelUse
	SetCompleteHook(0xE9, 0x00B63E91, &AllowJewelsInsert); //FOI PRO S16

	//CustomJewelShine
	SetCompleteHook(0xE9, 0x0069116C, &JewelsShine); //FOI PRO S16
	...
	char ClientIp[16];
	GetPrivateProfileString("LOGIN","IpAddress","192.168.1.3",ClientIp,sizeof(ClientIp),".\\Config.ini"); 
	MemoryCpy(0x01600560, ClientIp, sizeof(ClientIp));

	char ClientSerial[17];
	GetPrivateProfileString("LOGIN", "Serial", "0R9qSjeb4r1oRyuM", ClientSerial, sizeof(ClientSerial), ".\\Config.ini");
	...
	//Liberar opção 15 das asas/capas lvl 4
	...
	//DISABLE EFFECT +15
	MemorySet(0x006908EA, 0x90, 0x6AB);
