...
#define pScaleFormAction		((void(__cdecl*)(DWORD Form, LPSTR ElementName, LPSTR Command, ...)) 0x00A1C47B)//00A04DD3 | 00A1C47B | OLD: 0x009F4253?
#define pMessageBoxThis			((DWORD(__cdecl*)()) 0x00A2EBE0) //00A2EBE0? | Old: 0x00A0714B
...
struct PMSG_DEVILICE_MAGIC_SQUARE_POS
{
	PSBMSG_HEAD h;
	BYTE btStatus;
	BYTE btPosX1;
...
struct PMSG_MESSAGE_POPUP_RECV
{
	PSBMSG_HEAD h; // C1:F3:E2
	char message[1024];
};

struct HARDWARE_ID_SEND
{
	PBMSG_HEAD header;
	char HarwareId[45];
};
...
void CGSendHardWareId();

void InitProtocolCoreEx();
void DataSend(BYTE* pData, int size);
...
//void GCFourMasterLevelUpRecv(PMSG_4TH_MASTER_LEVEL_UP_RECV *lpMsg);
