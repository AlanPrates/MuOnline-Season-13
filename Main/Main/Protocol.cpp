...
BOOL ProtocolCoreEx(BYTE head, BYTE* lpMsg, int len, int Encrypt)
{
	...
	case 0xD2:
	{
		...
		{
			case 0x0C:
				PMSG_CASH_SHOP_VERSION_SEND * discountMsg = (PMSG_CASH_SHOP_VERSION_SEND*)lpMsg;
				Discount = discountMsg->Discount;
...
void CGSendHardWareId()
{
	...
	Log.ConsoleOutPut(1, c_Blue, t_Default, "HardDiskId %s", pResult.HarwareId);
