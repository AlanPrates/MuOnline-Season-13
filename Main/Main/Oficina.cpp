...
void Oficina::DrawOficinaConfirmWindow()
{
	...
	gInterface.DrawFormat(eGold, 245, 164, 166, eTxtAlign_center, 1, "OFICINA DE ITENS DO ICONEMU");
...
void Oficina::UpdatePrices()
{
	...
	if (CheckAllRepeatedOptions() || isPeriodic)
	{
		return;
	}
...
void Oficina::DrawOficinaPanelWindow()
{
	...
	//Título
	gInterface.DrawFormat(eGold, 181, 33, 258, eTxtAlign_center, 1, "OFICINA DE ITENS DO ICONEMU");
	...
	gInterface.DrawFormat(eShinyGreen, 319, 82, 130, eTxtAlign_left, 1, "%s%s",itemName, (isPeriodic) ? " (temporario)" : "");
	...
	if (
		(this->currentIcoinPrice > 0 || 
		(this->currentIcoinPrice == 0 &&
			isPeriodic && (
				this->originalOptionCode[0] != this->selectedOptionCode[0] ||
				this->originalOptionCode[1] != this->selectedOptionCode[1] ||
				this->originalOptionCode[2] != this->selectedOptionCode[2] ||
				this->originalOptionCode[4] != this->selectedOptionCode[4]
			)
		) ||
			(this->currentIcoinPrice == 0 &&
				CheckNonDesignedOption(this->originalOptionCode[0], itemType) &&
				CheckNonDesignedOption(this->originalOptionCode[1], itemType) &&
				CheckNonDesignedOption(this->originalOptionCode[2], itemType) &&
				CheckNonDesignedOption(this->originalOptionCode[4], itemType) &&
