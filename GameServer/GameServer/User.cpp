...
void gObjCharZeroSet(int aIndex) // OK
{
	...
	lpObj->FreezingDamageResist = 0;
	lpObj->FireingDamageRate = 0;
	lpObj->FireingDamageResist = 0;
	...
	lpObj->BackSpringIncrease = 0;
	lpObj->LastMoveSkillTimer = 0;

	lpObj->OficinaOpen = false;
	lpObj->OficinaItem = -1;
	ZeroMemory(&lpObj->OficinaOptions, sizeof(lpObj->OficinaOptions));

	lpObj->CashShopDiscount = 0;

	lpObj->ForcePvP = false;

	lpObj->PostOnOff = false;
}
...
void gObjTeleportMagicUse(int aIndex,int x,int y) // OK
{
	...
	lpObj->TeleportTime = GetTickCount64();
	lpObj->PathCount = 0;
	lpObj->Teleport = 1;
	lpObj->ViewState = 1;
	lpObj->X = x;
...
BOOL gObjMoveGate(int aIndex,int gate) // OK
{
	...
	if(TargetGate == 97) //portao da sala da coroa/elevador
	{
		...
		///////////////////////////////////
		//Contar membros na sala da coroa//
		///////////////////////////////////
		if (lpObj->Guild && gServerInfo.m_CastleSiegeMaxMembersInTower > 0)
		{			
			char szMasterGuildName[9];
			memset(&szMasterGuildName, 0, 9);

			//para guardar a guild principal nos loops
			GUILD_INFO_STRUCT* mainGuild;
			...
			for (int i = OBJECT_START_USER; i < MAX_OBJECT; i++)
			{
				...
				{
					if (gCastleSiege.IsInDragonTower(&gObj[i]))
					{
						LPOBJ lpTarget = &gObj[i];
						...
						if (lpTarget->Guild)
						{
							if (lpTarget->Guild->GuildUnion != 0)
							{
								...
								{
									szMasterGuildName[8] = '\0';

									mainGuild = gGuildClass.SearchGuild(szMasterGuildName);

									if (mainGuild != NULL)
									{
										mainGuild->membersInTower++;
										LogAdd(LOG_DEBUG, "Contando 1 (Portal) [%s] Membros: %d [%s][%d,%d]", mainGuild->Name, mainGuild->membersInTower, lpTarget->Name, lpTarget->X, lpTarget->Y);
									}
									else
									{
										lpTarget->Guild->membersInTower++;
										LogAdd(LOG_DEBUG, "Contando 2 (Portal) [%s] Membros: %d [%s][%d,%d]", lpTarget->Guild->Name, lpTarget->Guild->membersInTower, lpTarget->Name, lpTarget->X, lpTarget->Y);
									}
								}
							}
							else
							{
								lpTarget->Guild->membersInTower++;
								LogAdd(LOG_DEBUG, "Contando 3 (Portal) [%s] Membros: %d [%s][%d,%d]", lpTarget->Guild->Name, lpTarget->Guild->membersInTower, lpTarget->Name, lpTarget->X, lpTarget->Y);
							}
						}
					}
				}
			...
			//identificando a guild principal
			if (lpObj->Guild->GuildUnion != 0) //verificar se tem aliança
			{
				if (gUnionManager.GetUnionName(lpObj->Guild->GuildUnion, szMasterGuildName) != -1) //pegar o nome da aliança principal
				{
					szMasterGuildName[8] = '\0';

					mainGuild = gGuildClass.SearchGuild(szMasterGuildName); //nome da aliança principal

					if (mainGuild == NULL) //não achou nome da aliança principal
					{
						mainGuild = lpObj->Guild;
					}
				}
			}
			else
			{
				mainGuild = lpObj->Guild; //nao tendo aliança
			}

			LogAdd(LOG_DEBUG, "[%s] Membros: %d", mainGuild->Name, mainGuild->membersInTower);

			//comparando contagem
			if (mainGuild->membersInTower >= gServerInfo.m_CastleSiegeMaxMembersInTower)
			{
				gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "Limite de membros na sala da coroa atingido (%d)", mainGuild->membersInTower);

				if (lpObj->Guild->Index == mainGuild->Index && lpObj->GuildStatus == GUILD_MASTER)
				{
					gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "Entrada liberada para GMM");
				}
				else
				{
					goto ERROR_JUMP;
...
void gObjSummonAlly(LPOBJ lpObj,int map,int x,int y) // OK
{
	...
	if (map == MAP_CASTLE_SIEGE)
	{
		if (!gEffectManager.CheckEffect(lpObj, EFFECT_CALMA_MORENO))
		{
			gEffectManager.AddEffect(lpObj, 1, EFFECT_CALMA_MORENO, (int)(time(0) + 4), 0, 0, 0, 0);
		}
		else
		{
			CEffect* lpEffect = gEffectManager.GetEffect(lpObj, EFFECT_CALMA_MORENO);
			gEffectManager.AddEffect(lpObj, 1, EFFECT_CALMA_MORENO, (int)(lpEffect->m_time + 4), 0, 0, 0, 0);
		}
	}
...
bool gObjInventorySearchSerialNumber(LPOBJ lpObj,DWORD serial) // OK
{
	...
	for(int n=0;n < INVENTORY_SIZE;n++)
	{
		if(lpObj->Inventory[n].m_Serial != 0 && lpObj->Inventory[n].m_Serial == serial && (count++) > 0)
		{
			...
			GCMessagePopupSend(lpObj, "Encontramos um item duplicado em seu inventário, e ele foi automaticamente destruído.");
...
bool gObjWarehouseSearchSerialNumber(LPOBJ lpObj,DWORD serial) // OK
{
	...
	for(int n=0;n < WAREHOUSE_SIZE;n++)
	{
		...
		{
			GCMessagePopupSend(lpObj, "Encontramos um item duplicado em seu baú, e ele foi automaticamente destruído. Feche e reabra o baú para que ele suma de sua tela.");
...
void gObjSecondProc()
{
	...
	int urukBossZonePlayerCount = 0;
	bool isSilversterLive = false;
	...
	for(n = 0; n < MAX_OBJECT;n++)
	{
		...
		if(lpObj->Connected > OBJECT_LOGGED)
		{
			...
			if(lpObj->Type == OBJECT_MONSTER)
			{
				...
				if (lpObj->Class == 673 && lpObj->Live == 1)
				{
					isSilversterLive = true;
				}
			...
			if(lpObj->Type == OBJECT_USER)
			{
				if (lpObj->Map == MAP_URUK_MOUNTAIN)
				{
					if ((lpObj->X >= 110 && lpObj->X <= 148 && lpObj->Y >= 96 && lpObj->Y <= 120) ||
						(lpObj->X >= 112 && lpObj->X <= 141 && lpObj->Y >= 120 && lpObj->Y <= 128) ||
						(lpObj->X >= 124 && lpObj->X <= 140 && lpObj->Y >= 125 && lpObj->Y <= 139)
						)
					{
						urukBossZonePlayerCount++;
					}
				}
				
				/*if (((lpObj->Map == MAP_CASTLE_SIEGE && (lpObj->X > 40 || (lpObj->X < 40 && (lpObj->Y < 35 || lpObj->Y > 45)))
					|| lpObj->Map == MAP_DUNGEON)
					&& gItemManager.CheckInventoryRestrictions(lpObj)))
				{					
					gNotice.GCNoticeSend(lpObj->Index, 3, 0, 0, 0, 0, 0, "Apenas itens classicos sao permitidos no Siege nesta temporada. Consulte o site para mais informacoes.");
					gObjMoveGate(lpObj->Index, 17);
				}*/

				if (lpObj->Map == MAP_TEMPORADA) // Mapa da temporada, apenas um por PC
				{
					for (int i = OBJECT_START_USER; i < MAX_OBJECT; i++)
					{
						if (gObj[i].Connected > OBJECT_LOGGED && 
							gObj[i].Map == lpObj->Map &&							
							lpObj->Index != i && 
							strcmp(lpObj->HardDiskId, gObj[i].HardDiskId) == 0)
						{
							gNotice.GCNoticeSend(lpObj->Index, 3, 0, 0, 0, 0, 0, "Apenas um personagem por PC e permitido neste mapa");
							gNotice.GCNoticeSend(lpObj->Index, 3, 0, 0, 0, 0, 0, "Detectados: %s e %s", lpObj->Name, gObj[i].Name);
							gObjMoveGate(lpObj->Index, 17);
						}
					}
				}

				if (lpObj->Map == MAP_TEMPORADA) //Mapa da temporada, permanecer apenas se tiver ruud
				{
					/*if (lpObj->Map == MAP_DUNGEON && gItemManager.CheckInventoryRestrictions(lpObj))
					{
					...
					{
						gNotice.GCNoticeSend(lpObj->Index, 3, 0, 0, 0, 0, 0, "Para ficar neste mapa voce precisa ter ao menos %d Ruud", gServerInfo.m_PKRuudDrop);
						gObjMoveGate(lpObj->Index, 17);
					}
				}

				if (lpObj->ForcePvP == true && (GetTickCount64() - lpObj->ForcePvPTime) > 5000 )
				{
					gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "::: PvP do Noel desativado. :::");
					lpObj->ForcePvP = false;
				}
				...
				if (lpObj->MiningIndex > 0 && lpObj->MiningActionTime > 0 && GetTickCount64() - lpObj->MiningActionTime > 60000)
				{
					GCMessagePopupSend(lpObj, "Voce esta sendo desconectado por inatividade em ponto de mineracao.\nMais de um minuto parado. Tchau!");
					gObjUserKill(lpObj->Index);
				}
				...
				if(lpObj->Map == MAP_CASTLE_SIEGE && gCastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE)
				{
					...
					if (gServerInfo.m_CastleSiegeDecayAccumulatedTimeValue > 0 && gCastleSiege.GetCrownUserIndex() != lpObj->Index)
					{
						if (lpObj->m_LastClickCrownTime > 0 && lpObj->Guild > 0 && lpObj->Guild->AccumulatedCrownAccessTime > 0)
						{
							if ((GetTickCount() - lpObj->m_LastClickCrownTime) > (gServerInfo.m_CastleSiegeCrownIdleTimeout * 1000))
							{
								...
								lpObj->Guild->AccumulatedCrownAccessTime -= (gServerInfo.m_CastleSiegeDecayAccumulatedTimeValue * 1000);
								lpObj->Guild->AccumulatedCrownAccessTime = (lpObj->Guild->AccumulatedCrownAccessTime > 0) ? lpObj->Guild->AccumulatedCrownAccessTime : 0;
								lpObj->m_LastClickCrownTime = GetTickCount();

								gNotice.GCNoticeSendToAll(1, 0, 0, 0, 0, 0, "[%s] Acumulado perdido: %d seg. (%d)", lpObj->Name, gServerInfo.m_CastleSiegeDecayAccumulatedTimeValue, (lpObj->Guild->AccumulatedCrownAccessTime / 1000));

								gCastleSiege.SaveSiegeCharInfo(lpObj, 12, lpObj->Guild->AccumulatedCrownAccessTime);
							}
						}
					}
	...
	if (urukBossZonePlayerCount < gServerInfo.m_SilvesterMaxPlayers || isSilversterLive == false)
	{
		setActiveUrukTrapFalse();
	}
	else
	{
		setActiveUrukTrapTrue();
	}
}
