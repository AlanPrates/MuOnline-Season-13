...
bool CCommandManager::ManagementCore(LPOBJ lpObj, char* message) // OK
{
	...
	case COMMAND_REBUILD_TREE:
		...
		break;
	case COMMAND_PVPARTY:
		this->CommandPvParty(lpObj, argument);
		break;
	case COMMAND_CUPOM:
		this->CommandCupom(lpObj, argument);
		break;
	case COMMAND_POSTOFF:
		this->CommandPostOnOff(lpObj);
		break;
	default:
		return 0;
	}
...
void CCommandManager::CommandAddPoint(LPOBJ lpObj, char* arg, int type) // OK
{
	...
	if (lpObj->Class == CLASS_DL && type == 4 && lpObj->Map == MAP_CASTLE_SIEGE && 
		((lpObj->X < 30 || lpObj->X > 40) || (lpObj->Y < 30 || lpObj->Y > 45)))
	{
		GCMessagePopupSend(lpObj, "Em Loren, so e possivel adicionar pontos em comando no inicio do mapa.");
		return;
	}
...
void CCommandManager::CommandAutoPvP(LPOBJ lpObj, char* arg)
{
	...
	if (strcmp(lpObj->HardDiskId, lpTarget->HardDiskId) != 0)
	{
...
void CCommandManager::CommandPvParty(LPOBJ lpObj, char* arg)
{
	char name1[11] = { 0 };
	char name2[11] = { 0 };
	...
	if (strlen(name1) > 3 && strlen(name2) > 3)
	{
		...
		LPOBJ lpObjParty1 = gObjFind(name1);

		if (lpObjParty1 == 0)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Lutador da Party 1 nao encontrado");
			return;
		}

		if (lpObjParty1->PartyNumber < 0)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Lutador da Party 1 nao tem party");
			return;
		}

		LPOBJ lpObjParty2 = gObjFind(name2);

		if (lpObjParty2 == 0)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Lutador da Party 2 nao encontrado");
			return;
		}

		if (lpObjParty2->PartyNumber < 0)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Lutador da Party 2 nao tem party");
			return;
		}
		...
		for (int i = OBJECT_START_USER; i < MAX_OBJECT; i++)
		{
			...
			{
				if (gObj[i].PartyNumber == lpObjParty1->PartyNumber)
				{
					LogAdd(LOG_BLUE, "[GM][%s] Brought user %s to Party 1", lpObj->Name, lpObjParty1->Name);
					gObjTeleport(gObj[i].Index, 6, 202, 9 + broughtParty1);
					broughtParty1++;

					gEffectManager.ClearAllEffect(&gObj[i]);
					gEffectManager.AddEffect(&gObj[i], 0, EFFECT_ICE_ARROW, 10, 0, 0, 0, 0);
				}

				if (gObj[i].PartyNumber == lpObjParty2->PartyNumber)
				{
					LogAdd(LOG_BLUE, "[GM][%s] Brought user %s to Party 2", lpObj->Name, lpObjParty1->Name);
					gObjTeleport(gObj[i].Index, 6, 188, 9 + broughtParty2);
					broughtParty2++;
		...
		gLog.Output(LOG_COMMAND, "[CommandPvParty][%s][%s] - (Char 1: %s) (Char 2: %s)", lpObj->Account, lpObj->Name, lpObjParty1->Name, lpObjParty2->Name);
	}
}
...
void CCommandManager::CommandRuud(LPOBJ lpObj, char* arg)
{
	...
	case 500000:
		...
		break;
	case 1000000:
		item_type = GET_ITEM(14, 403);
		break;
	default:
		GCMessagePopupSend(lpObj, "Valor inválido. Tente 1000, 10000, 100000, 200000, 500000 ou 1000000");
		break;
	}
...
void CCommandManager::CommandRemoveWingOption(LPOBJ lpObj, char* arg)
{
	...
	{
		GCMessagePopupSend(lpObj, "Argumento inválido para este comando.\nDigite o comando seguido do numero do option (1 a 4) o qual deseja remover");
		return;
	}

	if (lpObj->Inventory[INVENTORY_WEAR_SIZE].IsItem() == 0)
	{
		GCMessagePopupSend(lpObj, "Coloque sua asa ou capa level 4 no primeiro espaço do inventario para utilizar este comando.");
		return;
	}
	...
	if (lpItem->m_Index < GET_ITEM(12, 414) || lpItem->m_Index > GET_ITEM(12, 421))
	{
		GCMessagePopupSend(lpObj, "Coloque sua uma asa ou capa level 4 no primeiro espaço do inventario para utilizar este comando.");
		return;
	}
	...
	{
		GCMessagePopupSend(lpObj, "Você não a quantidade de Ruud necessaria no momento (%d).", gServerInfo.m_CommandRemoveWingOptionRuud);
		return;
	}
...
void CCommandManager::CommandCupom(LPOBJ lpObj, char* arg)
{
	char dt_name[18] = { 0 };
	...
	if (strlen(dt_name) != 8)
	{
		GCMessagePopupSend(lpObj, "Digite um codigo de desconto valido.");
		return;
	}

	dt_name[8] = '\0';

	LOAD_CUPOM_SEND pMsg = { 0 };
	pMsg.h.set(0xD7, 25, sizeof(pMsg));
	pMsg.userIndex = lpObj->Index;
	memcpy(pMsg.cupomCode, dt_name, sizeof(pMsg.cupomCode));
	gDataServerConnection.DataSend((BYTE*)&pMsg, pMsg.h.size);
}

void CCommandManager::DGCommandLoadCupomRecv(LOAD_CUPOM_RECV* lpMsg) // OK
{
	if (OBJECT_RANGE(lpMsg->userIndex) == 0)
	{
		LogAdd(LOG_RED, "[DGCommandLoadCupomRecv] Invalid Index [%d]", lpMsg->userIndex);
		CloseClient(lpMsg->userIndex);
		return;
	}
	...
	if (lpMsg->discount == 0)
	{
		GCMessagePopupSend(lpObj, "Cupom invalido.");
		return;
	}

	//validações diversas

	lpObj->CashShopDiscount = lpMsg->discount;

	GCMessagePopupSend(lpObj, "Cupom ativado com sucesso!\nAbra o GameShop para ver e utilizar seu desconto.");

	gLog.Output(LOG_COMMAND, "[CommandLoadCupom][%s][%s] - (Discount: %d)", lpObj->Account, lpObj->Name, lpObj->CashShopDiscount);
}

void CCommandManager::CommandPostOnOff(LPOBJ lpObj) // OK
{
	if (lpObj->PostOnOff == false)
	{
		lpObj->PostOnOff = true;
		GCMessagePopupSend(lpObj, "A partir de agora, voce nao vera mensagens do post global");
	}
	else
	{
		lpObj->PostOnOff = false;
		GCMessagePopupSend(lpObj, "Mensagens do post global reativadas");
	}
}
