...
bool CItemManager::CheckItemMoveToBlock(LPOBJ lpObj, CItem* lpItem) // OK
{
	...
	{
		GCMessagePopupSend(lpObj, "Este item n�o pode ser movido de sua conta.");
		return 0;
	}	
...
bool CItemManager::CheckItemInventorySpace(LPOBJ lpObj, int width, int height, bool all) // OK
{
	int MaxY = (this->GetInventoryMaxValue(lpObj) - INVENTORY_WEAR_SIZE) / 8;
	...
	for (int y = 0; y < MaxY; y++)
	{
		for (int x = 0; x < 8; x++)
		{
			if (lpObj->InventoryMap[((y * 8) + x)] == 0xFF)
			{
				if (this->InventoryRectCheck(lpObj->Index, x, y, width, height, all) != 0xFF)
				{
					return 1;
				}
			}
...
BYTE CItemManager::InventoryRectCheck(int aIndex, int x, int y, int width, int height, bool all) // OK
{
...
BYTE CItemManager::MoveItemToTradeFromInventory(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
	...
	{
		GCMessagePopupSend(lpObj, "Voc� n�o pode transferir itens sem antes autenticar o PIN. Utilize o comando /pin seguido de seu PIN para faze-lo agora mesmo.");
		return 0xFF;
	}

	if (lpObj->Inventory[SourceSlot].m_Serial == 0 && lpObj->Authority != 32 && gObj[lpObj->TargetNumber].Authority != 32)
	{
		if (lpObj->Inventory[SourceSlot].m_Index != GET_ITEM(13, 31) &&
			lpObj->Inventory[SourceSlot].m_Index != GET_ITEM(10, 164) &&
			lpObj->Inventory[SourceSlot].m_Index != GET_ITEM(11, 165) &&
			lpObj->Inventory[SourceSlot].m_Index != GET_ITEM(11, 166) &&
			lpObj->Inventory[SourceSlot].m_Index != GET_ITEM(11, 167) &&
			lpObj->Inventory[SourceSlot].m_Index != GET_ITEM(11, 169) &&
			lpObj->Inventory[SourceSlot].m_Index != GET_ITEM(11, 171) &&
			lpObj->Inventory[SourceSlot].m_Index != GET_ITEM(11, 172) &&
			lpObj->Inventory[SourceSlot].m_Index != GET_ITEM(10, 189) &&
			lpObj->Inventory[SourceSlot].m_Index != GET_ITEM(11, 190) &&
			lpObj->Inventory[SourceSlot].m_Index != GET_ITEM(11, 191) &&
			lpObj->Inventory[SourceSlot].m_Index != GET_ITEM(11, 192) &&
			lpObj->Inventory[SourceSlot].m_Index != GET_ITEM(11, 193) &&
			lpObj->Inventory[SourceSlot].m_Index != GET_ITEM(11, 194) &&
			lpObj->Inventory[SourceSlot].m_Index != GET_ITEM(11, 195) &&
			lpObj->Inventory[SourceSlot].m_Index != GET_ITEM(11, 196) &&
			lpObj->Inventory[SourceSlot].m_Index != GET_ITEM(11, 197)
			)
		{
...
BYTE CItemManager::MoveItemToChaosBoxFromInventory(LPOBJ lpObj, BYTE SourceSlot, BYTE TargetSlot, BYTE TargetFlag) // OK
{
	...
	if (lpObj->Inventory[SourceSlot].m_Serial == 0 && lpObj->Authority != 32)
	{
		if (lpObj->Inventory[SourceSlot].m_Index != GET_ITEM(13, 31) && 
			lpObj->Inventory[SourceSlot].m_Index != GET_ITEM(10, 164) &&
			lpObj->Inventory[SourceSlot].m_Index != GET_ITEM(11, 165) &&
		   ...
		   )
		{
			...
			GCMessagePopupSend(lpObj, "Este item possui serial inv�lido e n�o pode ser combinado.");
			return 0xFF;
		}
	}
...
void CItemManager::CGItemGetRecv(PMSG_ITEM_GET_RECV* lpMsg, int aIndex) // OK
{
	...
	if (lpItem->m_Index == GET_ITEM(14, 352) || lpItem->m_Index == GET_ITEM(14, 336) ||
		...
		lpItem->m_Index == GET_ITEM(14, 403))
	{
...
void CItemManager::CGItemDropRecv(PMSG_ITEM_DROP_RECV* lpMsg, int aIndex) // OK
{
	...
	if (lpItem->m_Index == GET_ITEM(14, 134) ||
		lpItem->m_Index == GET_ITEM(14, 419) ||
		lpItem->m_Index == GET_ITEM(14, 139)
		)
	{
		if (gItemManager.CheckItemInventorySpace(lpObj, 8, 7, true) == false)
		{
			DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
			GCMessagePopupSend(lpObj, "Antes de utilizar essa box, deixe um bom espa�o limpo (8x8) em seu invent�rio.");
			return;
		}
	}
...
void CItemManager::CGItemUseRecv(PMSG_ITEM_USE_RECV* lpMsg, int aIndex) // OK
{
	...
	else if (lpItem->m_Index == GET_ITEM(14, 489)) //Jewel of Evolution
	{
		if (gCustomJewel.UseJewelOfEvolution(lpObj, lpMsg->SourceSlot, lpMsg->TargetSlot) != 0)
		{
			this->DecreaseItemDur(lpObj, lpMsg->SourceSlot, 1);
			this->GCItemModifySend(aIndex, lpMsg->SourceSlot);
			this->GCItemModifySend(aIndex, lpMsg->TargetSlot);
...
void CItemManager::GCItemDisintegration(PMSG_ITEM_DISINTEGRATION_RECV* lpMsg, int aIndex)
{
	...
	{
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		GCMessagePopupSend(lpObj, "Este item possui serial inv�lido e n�o pode ser desmontado.");
		return;
	}
