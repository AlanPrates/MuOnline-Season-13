...
void Oficina::NpcOpen(LPOBJ lpObj)
{
	...
	{
		if (lpItem->m_Index == GET_ITEM(6, 49))
		{
			pMsg.type = 22; //Shield Silver Heart

			if (!CheckValidOption(22, lpItem->m_SocketOption[0]) || !CheckValidOption(22, lpItem->m_SocketOption[1]) || !CheckValidOption(22, lpItem->m_SocketOption[2]) || !CheckValidOption(23, lpItem->m_SocketOption[4]))
			{
		...
		else if (lpItem->m_Index == GET_ITEM(6, 48))
		{
	...
	{
		if (lpItem->m_Index == GET_ITEM(0, 81) || //bk sword
			lpItem->m_Index == GET_ITEM(0, 82) || //mg sword
			lpItem->m_Index == GET_ITEM(0, 83) || //rf claw
			lpItem->m_Index == GET_ITEM(2, 42) || //dl scepter
			lpItem->m_Index == GET_ITEM(3, 30) || //gl lance
			lpItem->m_Index == GET_ITEM(4, 56) || //elf bow
			lpItem->m_Index == GET_ITEM(4, 57) || //elf quiver
			lpItem->m_Index == GET_ITEM(5, 64) || //dw staff
			lpItem->m_Index == GET_ITEM(5, 65) || //su stick
			lpItem->m_Index == GET_ITEM(5, 66)    //su book
			)
		{
			pMsg.type = 20; //Arma Silver Heart

			if (!CheckValidOption(20, lpItem->m_SocketOption[0]) || !CheckValidOption(20, lpItem->m_SocketOption[1]) || !CheckValidOption(20, lpItem->m_SocketOption[2]) || !CheckValidOption(21, lpItem->m_SocketOption[4]))
			{
				GCMessagePopupSend(lpObj, "O item que voce esta tentando modificar esta apropriado para modificacao, pois possui opcoes nao atualizadas.");
				return;
			}
		}
		else if (lpItem->m_Index == GET_ITEM(0, 78) || //bk sword
	...
	{
		if (ITEM_GET_INDEX(lpItem->m_Index) >= 189)
		{
			pMsg.type = 18; //Set Silver Heart

			if (!CheckValidOption(18, lpItem->m_SocketOption[0]) || !CheckValidOption(18, lpItem->m_SocketOption[1]) || !CheckValidOption(18, lpItem->m_SocketOption[2]) || !CheckValidOption(19, lpItem->m_SocketOption[4]))
			{
				GCMessagePopupSend(lpObj, "O item que voce esta tentando modificar esta apropriado para modificacao, pois possui opcoes nao atualizadas.");
				return;
			}
		}
		else if (ITEM_GET_INDEX(lpItem->m_Index) >= 164)
		{
			pMsg.type = 14; //Set Blue
...
void Oficina::BuyReceived(int aIndex, PMSG_OFICINA_CONFIRM_BUY_SEND* pMsg)
{
	...
	for (int i = 0; i < 5; i++)
	{
		...
		if (type != 0)
		{
			if (lpItem->m_SocketOption[i] == 254 || lpItem->m_IsPeriodicItem)
			{
				continue;
			}
		}
		...
		{
			if (lpItem->m_SocketOption[i] == 254 || lpItem->m_SocketOption[i] == 234 || lpItem->m_IsPeriodicItem)
			{
				continue;
			}
		}
		...
		if (pMsg->options[i] != lpItem->m_SocketOption[i])
		{
			if (i == 4)
			{
				...
				{
					gLog.Output(LOG_OFICINA, "[%s][%s] Op��o inv�lida para o Type %d : %d", lpObj->Account, lpObj->Name, (type+1), pMsg->options[i]);
					GCMessagePopupSend(lpObj, "Oops!\nAlgo de errado nao deu certo. :/\nErro: #007");
					CloseWindow(lpObj);
					return;
				}
			...
			else
			{
				...
				{
					gLog.Output(LOG_OFICINA, "[%s][%s] Op��o inv�lida para o Type %d : %d", lpObj->Account, lpObj->Name, type, pMsg->options[i]);
					GCMessagePopupSend(lpObj, "Oops!\nAlgo de errado nao deu certo. :/\nErro: #008");
					CloseWindow(lpObj);
					return;
				}
...
void Oficina::OnBuyCallbackRecv(LPOBJ lpObj, DWORD icoinCost, DWORD type, DWORD WCoinC, DWORD WCoinP, DWORD GoblinPoint) // OK
{
	...
	if (lpItem->m_Serial != lpObj->OficinaItem)
	{
		gLog.Output(LOG_OFICINA, "[%s][%s] Serial diferente p�s iCoins %d %d", lpObj->Account, lpObj->Name, lpItem->m_Serial, lpObj->OficinaItem);
		GCMessagePopupSend(lpObj, "Oops!\nAlgo de errado nao deu certo. :/\nErro: #006");
		gOficina.CloseWindow(lpObj);
		return;
	}
