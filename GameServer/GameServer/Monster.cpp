...
bool g_bActiveUrukTrigger;
DWORD g_dwTriggerTimer;

#define MAX_URUKTRIGGERSECTOR 6

const BYTE g_btUrukTriggerSector[MAX_URUKTRIGGERSECTOR][4] = {
	118, 90, 124, 96,
	127, 91, 136, 97,
	148, 105, 154, 112,
	139, 128, 144, 134,
...
const BYTE g_btUrukTriggerXY[MAX_URUKTRIGGERSECTOR][2] =
{
	120, 94,
	132, 96,
	150, 108,
...
const BYTE g_btDefUrukTriggerXY[MAX_URUKTRIGGERSECTOR][2] = {
	120, 92,
	135, 94,
	152, 108,
	143, 131,
...
void gObjMonsterDieGiveItem(LPOBJ lpObj, LPOBJ lpTarget) // OK
{
	...
	//Drop do ovo de ordnael
	if (lpTarget->AccountLevel > 0 && lpObj->isBoss)
	{
		if (lpObj->Class != 278 && lpObj->Class != 56 && lpObj->Class != 136 && lpObj->Class != 137 && lpObj->Class != 362 &&
			lpObj->Class != 363 && lpObj->Class != 131 && lpObj->Class != 132 && lpObj->Class != 133 && lpObj->Class != 134)
		{
			int dropChance = gServerInfo.OrdnaelEggDropRate;

			if (lpObj->Class == 476) //Noel
			{
				dropChance += 2;
			}

			if ((GetLargeRand() % 100) < dropChance)
			{
				GDCreateItemSend(lpTarget->Index, lpObj->Map, lpObj->X, lpObj->Y, GET_ITEM(14, 497), 0, 0, 0, 0, 0, lpTarget->Index, 0, 0, 0, 0, 0, 0, 0);
				gNotice.SendMapServerGroupMsg(0, "[%s] acaba de derrubar o Grande Ovo de Ordnael", lpTarget->Name);
				gLog.Output(ITEM_BAG_LOG, "[ItemBag][%s][%s] [Ordnael Big Egg]", lpTarget->Account, lpTarget->Name);
			}
		}

		if (lpObj->Class == 466 && lpObj->AutoPVPTesting != true && (GetLargeRand() % 100) < 10)
		{
			bool jaTemNoel = false;

			for (int n = 0; n < MAX_OBJECT_MONSTER; n++)
			{
				if (gObj[n].Class == 476 && gObj[n].DieRegen == 0) // Noel Malvadão
				{
					jaTemNoel = true;
				}
			}
			...
			if (jaTemNoel == false)
			{
				gNotice.SendMapServerGroupMsg(0, "Xiii, [%s] matou um Goblin Enzo, e la vem o papai dele!", lpTarget->Name);
				gNotice.SendMapServerGroupMsg(0, "Devias [%d,%d] (PvP On!)", lpObj->X, lpObj->Y);
				...
				if (OBJECT_RANGE(index) != 0)
				{
					...
					MonsterlpObj->Class = 476; //Noel malvadão

					MonsterlpObj->X = lpObj->X;
					MonsterlpObj->Y = lpObj->Y;
					MonsterlpObj->TX = lpObj->X;
					MonsterlpObj->TY = lpObj->Y;
					MonsterlpObj->OldX = lpObj->X;
					MonsterlpObj->OldY = lpObj->Y;
					MonsterlpObj->StartX = lpObj->X;
					MonsterlpObj->StartY = lpObj->Y;
					MonsterlpObj->Dir = (lpObj->Dir) % 8;
					MonsterlpObj->Map = lpObj->Map;

					MonsterlpObj->AutoPVPTesting = true; //gambi pra não permitir o super-spawn de super spawnadas

					if (gObjSetMonster(index, MonsterlpObj->Class, true) == 0)
					{
						gObjDel(index);
					}
				}
	...
	if (lpObj->Class == 746) // NIX
	{
		...
		{
			gItemBagManager.DropItemByMonsterClass(lpObj, lpTarget, lpObj->Map, lpObj->X, lpObj->Y);
		}

		return;
	}

	if (gItemBagManager.DropItemByMonsterClass(lpObj, lpTarget, lpObj->Map, lpObj->X, lpObj->Y) != 0)
	{
		return;
	}
	...
	int ItemDropRate = gMapManager.GetMapItemDropRate(lpObj->Map);

	if (ItemDropRate < 100 && lpObj->Money > 0 && (GetLargeRand() % ((lpObj->MoneyRate == 0) ? 1 : lpObj->MoneyRate)) < 10)
	{
...
bool gObjSetMonster(int aIndex, int MonsterClass, bool DeleteAfterDie, bool RespawnAfterDie, int Element) // OK
{
	if (OBJECT_RANGE(aIndex) == 0)
	{
		return 0;
	...
	if ((MonsterClass >= 764 && MonsterClass <= 766) ||
		(MonsterClass >= 658 && MonsterClass <= 668) ||
		MonsterClass == 669 || (MonsterClass >= 682 && MonsterClass <= 688) || MonsterClass == 669 || (MonsterClass >= 719 && MonsterClass <= 725) ||
		(MonsterClass >= 204 && MonsterClass <= 259) || (MonsterClass >= 367 && MonsterClass <= 371) || (MonsterClass >= 375 && MonsterClass <= 385) ||
		MonsterClass == 406 || MonsterClass == 407 || MonsterClass == 408 || MonsterClass == 415 || MonsterClass == 416 || MonsterClass == 417 ||
		(MonsterClass >= 450 && MonsterClass <= 453) || MonsterClass == 464 || MonsterClass == 465 || (MonsterClass >= 467 && MonsterClass <= 475) ||
		MonsterClass == 478 || MonsterClass == 479 || MonsterClass == 492 || MonsterClass == 522 || (MonsterClass >= 540 && MonsterClass <= 547) ||
		(MonsterClass >= 566 && MonsterClass <= 568) || (MonsterClass >= 577 && MonsterClass <= 584) || MonsterClass == 604 || MonsterClass == 643 ||
		MonsterClass == 651 || MonsterClass == 748)
	{
	...
	lpObj->OriginalRegenTime = lpInfo->RegenTime * 1000;
	...
	lpObj->m_MPSkillOpt.Resistance[SKILL_ELEMENT_ICE] = ((lpInfo->Resistance[SKILL_ELEMENT_ICE] > 255) ? 255 : lpInfo->Resistance[SKILL_ELEMENT_ICE]);
	lpObj->m_MPSkillOpt.Resistance[SKILL_ELEMENT_POISON] = ((lpInfo->Resistance[SKILL_ELEMENT_POISON] > 255) ? 255 : lpInfo->Resistance[SKILL_ELEMENT_POISON]);
	lpObj->m_MPSkillOpt.Resistance[SKILL_ELEMENT_LIGHTNING] = ((lpInfo->Resistance[SKILL_ELEMENT_LIGHTNING] > 255) ? 255 : lpInfo->Resistance[SKILL_ELEMENT_LIGHTNING]);
	lpObj->m_MPSkillOpt.Resistance[SKILL_ELEMENT_FIRE] = ((lpInfo->Resistance[SKILL_ELEMENT_FIRE] > 255) ? 255 : lpInfo->Resistance[SKILL_ELEMENT_FIRE]);
	lpObj->m_MPSkillOpt.Resistance[SKILL_ELEMENT_EARTH] = ((lpInfo->Resistance[SKILL_ELEMENT_EARTH] > 255) ? 255 : lpInfo->Resistance[SKILL_ELEMENT_EARTH]);
	lpObj->m_MPSkillOpt.Resistance[SKILL_ELEMENT_WIND] = ((lpInfo->Resistance[SKILL_ELEMENT_WIND] > 255) ? 255 : lpInfo->Resistance[SKILL_ELEMENT_WIND]);
	lpObj->m_MPSkillOpt.Resistance[SKILL_ELEMENT_WATER] = ((lpInfo->Resistance[SKILL_ELEMENT_WATER] > 255) ? 255 : lpInfo->Resistance[SKILL_ELEMENT_WATER]);
	...
	{
		lpObj->ElementalAttribute = Element;
	}
...
bool gObjMonsterRegen(LPOBJ lpObj) // OK
{
	...
	{
		lpObj->ConnectTickCount = GetTickCount();
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, "[Golden Kundun] Dungeon 3 pra que?");
	}*/
...
int gObjMonsterGetDamageByUser(LPOBJ lpMonObj, int aIndex)
{
	for (int i = 0; i < MAX_HIT_DAMAGE; i++)
	{
		if (lpMonObj->HitDamage[i].index == aIndex)
			return i;
	}
...
void gObjSummonKill(int aIndex) // OK
{
	...
	LogAdd(LOG_RED, "gObjDel %s %s %d [Class: %d]", __FILE__, __FUNCTION__, __LINE__, gObj[lpSummon->Index].Class);
...
void gObjMonsterDie(LPOBJ lpObj, LPOBJ lpTarget) // OK
{
	...
	if (lpObj->Class == 747)
	{
		LogAdd(LOG_GREEN, "[NixiesBoss][SummonRestrictStone] Removed");
		g_NixiesLakeBoss.RemoveRestrictStone(lpObj);
	}

	if (lpObj->Class == 746)
	{
		LogAdd(LOG_GREEN, "NIXIES BOSS ZONE : Nix Die");
...
void gObjMonsterStateProc(LPOBJ lpObj, int code, int aIndex, int SubCode) // OK
{
	...
	case 3:
		lpObj->TargetNumber = -1;
		lpObj->LastAttackerID = -1;
		lpObj->NextActionTime = 100; // TRY 1000
		lpObj->ActionState.Attack = 0;
		lpObj->ActionState.Move = 0;
		lpObj->ActionState.Emotion = 0;
		break;
	...
	case 56:
		if (gObjCheckResistance(&gObj[aIndex], SKILL_ELEMENT_POISON) == 0)
		{
...
void gObjMonsterProcess(LPOBJ lpObj)
{
	...
	if (lpObj->Class == 746)
	{
		if (g_NixiesLakeBossZone.GetBossZoneState() == 0) //tem ninguém
		{
		...
		else if (g_NixiesLakeBossZone.GetBossZoneState() == 1) //tem player
		{
			if (g_NixiesLakeBossZone.ChkBossZonePlayer() == 0)
			{
				g_NixiesLakeBossZone.SetBossZoneState(0);
				return;
			}
		}
		else if (g_NixiesLakeBossZone.GetBossZoneState() == 2)
		{
			return;
		}
	...
	else if (lpObj->Class == 704 || lpObj->Class == 705 || lpObj->Class == 708 || lpObj->Class == 735 || lpObj->Class == 747)
	{
		return;
	}
	else
	...
	if (lpObj->ActionState.Attack == 1 || lpObj->isBoss)
	{
	...
	if (lpObj->ActionState.Move != 0 && lpObj->Class != 673 && lpObj->Class != 476)
	{
	...
	while (searchc-- != 0)
	{
		...
		else
		{
			if ((attr & 1) != 1 && (attr & 2) != 2 && (attr & 4) != 4 && (attr & 8) != 8 && mchk)
			{
				lpObj->TargetNumber = -1;
				lpObj->ActionState.Attack = 0;
				lpObj->NextActionTime = 100; // TRY 400
				lpObj->ActionState.Emotion = 0;
				lpObj->MTX = tpx;
				lpObj->MTY = tpy;
				lpObj->ActionState.Move = 1;
...
void gObjMonsterBaseAct(LPOBJ lpObj)
{
	...
	{
		lpTargetObj = &gObj[lpObj->TargetNumber];
		//LogAdd(LOG_BLUE, "Target: %s", lpTargetObj->Name);
	}
	else
	{
		//LogAdd(LOG_DEBUG, "Sem target");
		lpObj->ActionState.Emotion = 0;
	}
	...
	//LogAdd(LOG_RED, "Emotion: %d", lpObj->ActionState.Emotion);
	...
	if (lpObj->ActionState.Emotion == 0)
	{
		if (lpObj->Attribute)
		{
			...
			if (actcode1 == 0)
			{
				...
				lpObj->NextActionTime = 100; // TRY 500
			}
			...
			{
				if (lpObj->Attribute == 100) //Summon/Illusion
				{
			...
			if (lpObj->Class == 673)
			{
				if (lpObj->VPCount2 < 2)
				{
					if (g_bActiveUrukTrigger == true && (GetTickCount() - g_dwTriggerTimer) > 60000)
					{
						if (lpObj->MaxLife > lpObj->Life)
						{
							lpObj->Life = 1000000 + lpObj->Life;
						}
					}
				}
			}
	...
	else if (lpObj->ActionState.Emotion == 2)
	{
		...
		lpObj->ActionState.Move = 1;
		lpObj->NextActionTime = 100; // TRY 800
...
void gAllSendUrukBlockInfo(bool bLive)
{	
	BYTE cTEMP_BUF[256];
	...
	cTEMP_BUF[6] = 6;
	cTEMP_BUF[4] = 4;
	cTEMP_BUF[5] = bLive == FALSE;

	int nIndex = 0;

	for (int nTrrigerIdx = 0; nTrrigerIdx < 6; nTrrigerIdx++)
	{
		cTEMP_BUF[2 * nIndex + 7] = g_btUrukTriggerSector[nTrrigerIdx][0];
		cTEMP_BUF[2 * nIndex + 8] = g_btUrukTriggerSector[nTrrigerIdx][1];
		nIndex++;
		cTEMP_BUF[2 * nIndex + 7] = g_btUrukTriggerSector[nTrrigerIdx][2];
		cTEMP_BUF[2 * nIndex + 8] = g_btUrukTriggerSector[nTrrigerIdx][3];
		nIndex++;
	}

	for (int n = OBJECT_START_USER; n < MAX_OBJECT; ++n)
	{
		if (gObj[n].Map == MAP_URUK_MOUNTAIN && gObj[n].Connected > OBJECT_LOGGED)
		{
			DataSend(n, cTEMP_BUF, cTEMP_BUF[1]);
		}
	}
}

void gSetUrukTriggerRelease()
{
	for (int triggerNum = 0; triggerNum < 6; triggerNum++)
	{
		for (int i = g_btUrukTriggerSector[triggerNum][0]; i <= g_btUrukTriggerSector[triggerNum][2]; i++)
		{
			for (int j = g_btUrukTriggerSector[triggerNum][1]; j <= g_btUrukTriggerSector[triggerNum][3]; j++)
			{
				gMap[MAP_URUK_MOUNTAIN].m_MapAttr[(j * 256) + i] &= ~4;
			}
		}
	}

	gAllSendUrukBlockInfo(FALSE);
	g_bActiveUrukTrigger = FALSE;
}

void gSetUrukTriggerBlock()
{
	BOOL bIsStandChar = FALSE;

	for (int triggerNum = 0; triggerNum < 6; triggerNum++)
	{
		for (int x = g_btUrukTriggerSector[triggerNum][0]; x <= g_btUrukTriggerSector[triggerNum][2]; x++)
		{
			for (int y = g_btUrukTriggerSector[triggerNum][1]; y <= g_btUrukTriggerSector[triggerNum][3]; y++)
			{
				gMap[MAP_URUK_MOUNTAIN].m_MapAttr[(y * 256) + x] |= 4;

				if ((gMap[MAP_URUK_MOUNTAIN].GetAttr(x, y) & 0x02) == 0x02)
				{
					bIsStandChar = TRUE;
				}
			}
	...
	if (bIsStandChar == TRUE)
	{
		gObjPushUrukTriggerOut();
	}

	gAllSendUrukBlockInfo(TRUE);
	g_bActiveUrukTrigger = TRUE;
	g_dwTriggerTimer = GetTickCount();
}

void gObjPushUrukTriggerOut()
{
	for (int nIndex = 0; nIndex < MAX_OBJECT; nIndex++)
	{
		if (gObj[nIndex].Map == MAP_URUK_MOUNTAIN && gObj[nIndex].Connected > OBJECT_LOGGED)
		{
			int triggerNum = gIsUrukTriggerSector(gObj[nIndex].X, gObj[nIndex].Y);
			...
			if (triggerNum > -1)
			{
				...
				btDir = 2 * GetPathPacketDirPos(x - g_btUrukTriggerXY[triggerNum][0], y - g_btUrukTriggerXY[triggerNum][1]);
				int count = 0;
				...
				while (true)
				{
					...
					tx = x + 2 * RoadPathTable[btDir];
					ty = y + 2 * RoadPathTable[btDir + 1];

					BYTE attr = gMap[MAP_URUK_MOUNTAIN].GetAttr(tx, ty);

					if (!(attr & 1) && !(attr & 4) && !(attr & 8) && !(attr & 0x10))
					{
						break;
					}
					...
					if (count >= 8)
					{
						x = g_btDefUrukTriggerXY[triggerNum][0];
						y = g_btDefUrukTriggerXY[triggerNum][1];
						break;
					}
...
int gIsUrukTriggerSector(int X, int Y)
{
	if (X >= g_btUrukTriggerSector[0][0] &&
		X <= g_btUrukTriggerSector[0][2] &&
		Y >= g_btUrukTriggerSector[0][1] &&
		Y <= g_btUrukTriggerSector[0][3])
	{
		return 0;
	}

	if (X >= g_btUrukTriggerSector[1][0] &&
		X <= g_btUrukTriggerSector[1][2] &&
		Y >= g_btUrukTriggerSector[1][1] &&
		Y <= g_btUrukTriggerSector[1][3])
	{
		return 1;
	}

	if (X >= g_btUrukTriggerSector[2][0] &&
		X <= g_btUrukTriggerSector[2][2] &&
		Y >= g_btUrukTriggerSector[2][1] &&
		Y <= g_btUrukTriggerSector[2][3])
	{
		return 2;
	}

	if (X >= g_btUrukTriggerSector[3][0] &&
		X <= g_btUrukTriggerSector[3][2] &&
		Y >= g_btUrukTriggerSector[3][1] &&
		Y <= g_btUrukTriggerSector[3][3])
	{
		return 3;
	}

	if (X >= g_btUrukTriggerSector[4][0] &&
		X <= g_btUrukTriggerSector[4][2] &&
		Y >= g_btUrukTriggerSector[4][1] &&
		Y <= g_btUrukTriggerSector[4][3])
	{
		return 4;
	}

	if (X >= g_btUrukTriggerSector[5][0] &&
		X <= g_btUrukTriggerSector[5][2] &&
		Y >= g_btUrukTriggerSector[5][1] &&
		Y <= g_btUrukTriggerSector[5][3])
	{
		return 5;
	}
...
void setActiveUrukTrapFalse()
{
	g_bActiveUrukTrap = false;
	gSetUrukTriggerRelease();
}

void setActiveUrukTrapTrue()
{
	g_bActiveUrukTrap = true;
	gSetUrukTriggerBlock();
}

bool CheckUrukBossZone()
{
	int playerCount = 0;
	...
	for (int i = OBJECT_START_USER; i < MAX_OBJECT; i++)
	{
		...
		{
			if (gObj[i].Map == MAP_URUK_MOUNTAIN)
			{
				if (gObj[i].X >= 113 && gObj[i].X <= 143 && gObj[i].Y >= 99 && gObj[i].Y <= 128)
				{
					playerCount++;
				}
			}
