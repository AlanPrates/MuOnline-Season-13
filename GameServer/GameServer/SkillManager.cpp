...
void CSkillManager::LoadSkillDamageRate(char* path)
{
	...
	for (int n = 0; n < 64; n++)
	{
		this->SkillDamageRate[n].Class = -1;
		this->SkillDamageRate[n].Skill = -1;
	}
	...
	try
	{
		while (true)
		{
			...
			this->SkillDamageRate[count].Class = lpMemScript->GetNumber();
			this->SkillDamageRate[count].Skill = lpMemScript->GetAsNumber();
			this->SkillDamageRate[count].DamageRate = lpMemScript->GetAsNumber();
...
bool CSkillManager::CheckSkillTarget(LPOBJ lpObj, int aIndex, int bIndex, int type) // OK
{
	...
	//Massacre Mapa Temporada, skill de area pega em todos
	if (lpObj->Map == MAP_TEMPORADA && lpTarget->Map == MAP_TEMPORADA)
	{
	...
	{
		if (lpObj->X >= 185 && lpObj->Y >= 8 && lpObj->X <= 204 && lpObj->Y <= 16)
		{
			if (lpTarget->X >= 185 && lpTarget->Y >= 8 && lpTarget->X <= 204 && lpTarget->Y <= 16)
			{
				if (lpObj->PartyNumber == lpTarget->PartyNumber)
				{
					return 0;
...
bool CSkillManager::RunningSkill(int aIndex, int bIndex, CSkill* lpSkill, BYTE x, BYTE y, BYTE angle, bool combo) // OK
{
	...
	int theSkill = gSkillManager.GetBaseSkill(lpSkill->m_skill);
	...
	case SKILL_CHAIN_DRIVER:
		lpObj->LastMoveSkillTimer = GetTickCount64();
		break;
	}
	...
	case SKILL_SHADOW_STEP:
		return false; //this->SkillShadowStep(aIndex, bIndex, lpSkill);
	case SKILL_EVASION:
...
bool CSkillManager::SkillDefense(int aIndex, int bIndex, CSkill* lpSkill) // OK
{
	...
	if (lpObj->Inventory[1].IsItem() != 0)
	{
		if (lpObj->Inventory[1].m_Index >= GET_ITEM(6, 0) && lpObj->Inventory[1].m_Index < GET_ITEM(7, 0))
		{
			...
			{
				lpObj->ShieldDamageReductionTime = GetTickCount64();
...
bool CSkillManager::SkillSummonParty(int aIndex, int bIndex, CSkill* lpSkill) // OK
{
	...
	if (OBJECT_RANGE(lpObj->PartyNumber) != 0)
	{
		...
		///////////////////////////////////
		if (gCastleSiege.IsInDragonTower(lpObj) && gServerInfo.m_CastleSiegeMaxMembersInTower > 0)
		{
			//Zera tudo
				...
				if (gObj[i].Connected == OBJECT_ONLINE && gObj[i].Type == OBJECT_USER)
				{
					if (gCastleSiege.IsInDragonTower(&gObj[i]))
					{
						...
						if (lpTarget->Guild)
						{
							if (lpTarget->Guild->GuildUnion != 0)
							{
								...
								if (gUnionManager.GetUnionName(lpTarget->Guild->GuildUnion, szMasterGuildName) != -1)
								{
									...
									if (mainGuild != NULL)
									{
										...
										LogAdd(LOG_DEBUG, "Contando 1 (Summon) [%s] Membros: %d [%s][%d,%d]", mainGuild->Name, mainGuild->membersInTower, lpTarget->Name, lpTarget->X, lpTarget->Y);
									}
									else
									{
										lpTarget->Guild->membersInTower++;
										LogAdd(LOG_DEBUG, "Contando 2 (Summon) [%s] Membros: %d [%s][%d,%d]", lpTarget->Guild->Name, lpTarget->Guild->membersInTower, lpTarget->Name, lpTarget->X, lpTarget->Y);
									}
								}
							}
							else
							{
								lpTarget->Guild->membersInTower++;
								LogAdd(LOG_DEBUG, "Contando 3 (Summon) [%s] Membros: %d [%s][%d,%d]", lpTarget->Guild->Name, lpTarget->Guild->membersInTower, lpTarget->Name, lpTarget->X, lpTarget->Y);
							}
						}
					}
				}
		...
		for (int n = 0; n < MAX_PARTY_USER; n++)
		{
			...
			if (lpObj->Map == MAP_CASTLE_SIEGE && gCastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE)
			{
				...
				if (lpTarget->Class == CLASS_DL && lpTarget->Leadership > 1000)
				{
					gNotice.GCNoticeSend(index, 1, 0, 0, 0, 0, 0, "O transporte foi cancelado [mais de 1.000 pontos em Comando].");
					continue;
				}

				//identificando a guild principal do puxado
				if (gCastleSiege.IsInDragonTower(lpObj) && gServerInfo.m_CastleSiegeMaxMembersInTower > 0)
				{
					...
					{
						gNotice.GCNoticeSend(index, 1, 0, 0, 0, 0, 0, "Limite de membros na sala da coroa atingido (%d)", mainGuild->membersInTower);
						gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "Limite de membros na sala da coroa atingido (%d)", mainGuild->membersInTower);

						if (lpTarget->Guild->Index == mainGuild->Index && lpTarget->GuildStatus == GUILD_MASTER)
						{
							gNotice.GCNoticeSend(index, 1, 0, 0, 0, 0, 0, "Entrada liberada para GMM");
...
bool CSkillManager::SkillLesserDefense(int aIndex, int bIndex, CSkill* lpSkill) // OK
{
	...
	for (int n = 0; n < MAX_VIEWPORT; n++)
	{
		...
		if (gObj[index].Map == MAP_CASTLE_SIEGE && gObj[index].X >= 30 && gObj[index].Y >= 30 && gObj[index].X <= 40 && gObj[index].Y <= 45)
		{
			continue;
		}
...
{
	if (GetLargeRand() % 100 < (lpTarget->BackSpringProtection - lpObj->BackSpringIncrease))
	{
		return;
	}
...
void CSkillManager::ApplyDragonLoreEffect(LPOBJ lpObj, LPOBJ lpTarget, CSkill* lpSkill, int damage) // OK
{
	...
	{
		damage = 100;
		gEffectManager.AddEffect(lpTarget,0,EFFECT_FIREING_DAMAGE,10,lpObj->Index,1,SET_NUMBERHW(damage),SET_NUMBERLW(damage));
	}*/
...
void CSkillManager::CGSkillAttackRecv(PMSG_SKILL_ATTACK_RECV* lpMsg, int aIndex) // OK
{
	...
	if (lpObj->Type == OBJECT_USER)
	{
		...
		if (this->CheckSkillDelay(lpObj, lpSkill->m_index) == 0)
		{
			if (lpSkill->m_index == SKILL_SHADOW_STEP)
			{
...
int CSkillManager::GetDamageRatePvP(LPOBJ lpObj, int Skill)
{
	Skill = this->GetBaseSkill(Skill);

	for (int n = 0; n < 64; n++)
	{
		if (this->SkillDamageRate[n].Skill == -1)
		{
			continue;
		}

		if (this->SkillDamageRate[n].Skill == Skill && (lpObj->Class == -1 || lpObj->Class == this->SkillDamageRate[n].Class))
		{
			return this->SkillDamageRate[n].DamageRate;
		}
	}
