				...
				if ((it->second.DropInfo[0].OptionValue & 4) != 0 && this->CheckDropClass(lpObjRewarded, it->second.DropInfo[0].RequireClass) != 0) //todos os itens da lista
				{
					for (int buffIndex = 0; buffIndex < ItemInfo->second.size(); buffIndex++)
					{
						...
						if ((it->second.DropInfo[0].OptionValue & 8) != 0) //pegar sockets
						{
							...
							GDCreateItemSend(lpObjRewarded->Index, 0xEB, 0, 0, ItemIndex, ItemLevel, 255, ItemOption1, ItemOption2, ItemOption3, lpObjRewarded->Index, ItemNewOption, ItemSetOption, 0, 0, ItemSocketOption, ItemSocketOption[5], (lpItemBagItemInfo->Duration > 0) ? ((DWORD)time(0) + lpItemBagItemInfo->Duration) : 0);
						}
						else
						{
							BYTE Durability = 255;
							...
							GDCreateItemSend(lpObjRewarded->Index, 0xEB, 0, 0, ItemIndex, ItemLevel, Durability, 0, 0, 0, lpObjRewarded->Index, 0, 0, 0, 0, 0, 0, 0);
						}

						char itemName[64];
						strcpy_s(itemName, gItemManager.GetName(ItemIndex));

						gLog.Output(ITEM_BAG_LOG, "[ItemBagEx][%s][%s][Package Item][%s]", lpObjRewarded->Account, lpObjRewarded->Name, itemName);
					}
				}
				else
				{
					...
					BYTE ItemNewOption = 0;
					BYTE ItemSetOption = 0;
					BYTE ItemSocketOption[MAX_SOCKET_OPTION+1] = { 0xFF,0xFF,0xFF,0xFF,0xFF,0xFF };

					if ((lpItemBagDropInfo->OptionValue & 8) != 0) //pegar sockets
					{
						ItemNewOption = lpItemBagItemInfo->Grade;
						ItemSetOption = lpItemBagItemInfo->Option0;
					...
					if (lpObjRewarded->MouseItemUp == 1)
					{
						...
						else
						{
							...
							GDCreateItemSend(lpObjRewarded->Index, 0xEB, px, py, ItemIndex, ItemLevel, item.m_Durability, ItemOption1, ItemOption2, ItemOption3, lpObjRewarded->Index, ((ItemNewOption == 0) ? lpItemBagItemInfo->Grade : ItemNewOption), ItemSetOption, 0, 0, ItemSocketOption, ItemSocketOption[5], ((lpItemBagItemInfo->Duration > 0 && item.m_Durability == 1) ? ((DWORD)time(0) + lpItemBagItemInfo->Duration) : 0));
