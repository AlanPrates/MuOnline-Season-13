...
bool CMysteriousStone::ApplyOnSet(LPOBJ lpObj, int SourceSlot, int TargetSlot) // OK
{
	...
	if (ITEM_GET_INDEX(lpObj->Inventory[TargetSlot].m_Index) >= 189 && ITEM_GET_INDEX(lpObj->Inventory[TargetSlot].m_Index) <= 197)
	{
		return ApplyOnSilverHeartSet(lpObj, SourceSlot, TargetSlot);
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 3, 0, 0, 0, 0, 0, "A aplicacao de maestria falhou.");
		return 1;
	}
...
bool CMysteriousStone::ApplyOnSilverHeartSet(LPOBJ lpObj, int SourceSlot, int TargetSlot) // OK
{
	if (GetLargeRand() % 100 > gServerInfo.m_MysteriouStoneSilverHeartSetApplyRate)
	{
		gNotice.GCNoticeSend(lpObj->Index, 3, 0, 0, 0, 0, 0, "A aplicacao de maestria falhou.");
		return 1;
	}
	...
	for (int n = 0; n < 3; n++)
	{
		...
		else if (lpItem->m_SocketOption[n] >= 6 && lpItem->m_SocketOption[n] <= 15) //já tem opcional muito antigo
		{
			lpItem->m_SocketOption[n] += 40;
		...
		else if (lpItem->m_SocketOption[n] >= 46 && lpItem->m_SocketOption[n] <= 55) //já tem opcional antigo
		{
			lpItem->m_SocketOption[n] += 24;
	...
	if (lpItem->m_SocketOption[4] >= 1 && lpItem->m_SocketOption[4] <= 4) //tem option antigo
	{
		lpItem->m_SocketOption[4] += 9; //evolui pro add do silver
	}
	else
	{
		if (lpItem->m_SocketOption[4] != 10)
...
bool CMysteriousStone::ApplyOnWeapon(LPOBJ lpObj, int SourceSlot, int TargetSlot) // OK
{
	...
	else if (lpObj->Inventory[TargetSlot].m_Index == GET_ITEM(0, 81) || //bk sword
		lpObj->Inventory[TargetSlot].m_Index == GET_ITEM(0, 82) || //mg sword
		lpObj->Inventory[TargetSlot].m_Index == GET_ITEM(0, 83) || //rf claw
		lpObj->Inventory[TargetSlot].m_Index == GET_ITEM(2, 42) || //dl scepter
		lpObj->Inventory[TargetSlot].m_Index == GET_ITEM(3, 30) || //gl lance
		lpObj->Inventory[TargetSlot].m_Index == GET_ITEM(4, 56) || //elf bow
		lpObj->Inventory[TargetSlot].m_Index == GET_ITEM(4, 57) || //elf quiver
		lpObj->Inventory[TargetSlot].m_Index == GET_ITEM(5, 64) || //dw staff
		lpObj->Inventory[TargetSlot].m_Index == GET_ITEM(5, 65) || //su stick
		lpObj->Inventory[TargetSlot].m_Index == GET_ITEM(5, 66)    //su book
		)
	{
		return ApplyOnSilverHeartWeapon(lpObj, SourceSlot, TargetSlot);
	}
...
bool CMysteriousStone::ApplyOnShield(LPOBJ lpObj, int SourceSlot, int TargetSlot) // OK
{
	...
	else if (lpObj->Inventory[TargetSlot].m_Index == GET_ITEM(6, 49)) //silver heart
	{
		return ApplyOnSilverHeartShield(lpObj, SourceSlot, TargetSlot);
	}
...
bool CMysteriousStone::ApplyOnRing(LPOBJ lpObj, int SourceSlot, int TargetSlot) // OK
{
	...
	RandomOptions.AddElement(40, 10);
	...
	RandomBonus.AddElement(26, 10);
	RandomBonus.AddElement(27, 10);
...
bool CMysteriousStone::ApplyOnPendant(LPOBJ lpObj, int SourceSlot, int TargetSlot) // OK
{
	...
	RandomOptions2.AddElement(45, 10);
	...
	if (luckyNumber < 20) //full 20%
	{
		...
		RandomBonus2.AddElement(25, 105);
...
bool CMysteriousStone::ApplyOnSilverHeartWeapon(LPOBJ lpObj, int SourceSlot, int TargetSlot) // OK
{
	if (GetLargeRand() % 100 > gServerInfo.m_MysteriouStoneSilverHeartWeaponApplyRate)
	{
		gNotice.GCNoticeSend(lpObj->Index, 3, 0, 0, 0, 0, 0, "A aplicacao de maestria falhou.");
		return 1;
	}
	...
	for (int n = 0; n < 3; n++)
	{
		if (lpItem->m_SocketOption[n] < 8 || lpItem->m_SocketOption[n] == 0xFF) //não tem opcional de arma ainda
		{
			CRandomManager RandomOptions;

			for (int y = 37; y <= 44; y++) //intervalo de options da arma Silver Heart
			{
				bool jatem = false;
		...
		else if (lpItem->m_SocketOption[n] >= 16 && lpItem->m_SocketOption[n] <= 23) //já tem opcional blue
		{
			lpItem->m_SocketOption[n] += 21; //intervalo da blue eye pra silver
	...
	if (lpItem->m_SocketOption[4] == 1 || (lpItem->m_SocketOption[4] >= 3 && lpItem->m_SocketOption[4] <= 7)) //tem BONUS antigo
	{
		lpItem->m_SocketOption[4] += 7; //evolui pro add do blue eye
	}
	else if (lpItem->m_SocketOption[4] >= 8 && lpItem->m_SocketOption[4] <= 14) //tem BONUS blue eye
	{
		lpItem->m_SocketOption[4] += 22; //evolui pro add do silver
	}
	else
	{
		CRandomManager RandomOptions;

		if (lpItem->m_SocketOption[4] != MASTERY_BONUS_ULTRAHIT_50_WEAPON_3)
			RandomOptions.AddElement(MASTERY_BONUS_ULTRAHIT_50_WEAPON_3, 10); //50 porc

		if (lpItem->m_SocketOption[4] != MASTERY_BONUS_100P_EXCELLENT)
			RandomOptions.AddElement(MASTERY_BONUS_100P_EXCELLENT, 10); //100% exc

		if (lpItem->m_SocketOption[4] != MASTERY_BONUS_FREEZING_DAMAGE_2)
			RandomOptions.AddElement(MASTERY_BONUS_FREEZING_DAMAGE_2, 10); //dano congelante

		if (lpItem->m_SocketOption[4] != MASTERY_BONUS_REFLECT_DECREASE_3)
			RandomOptions.AddElement(MASTERY_BONUS_REFLECT_DECREASE_3, 10); //abs reflect

		if (lpItem->m_SocketOption[4] != MASTERY_BONUS_CURE_HARM_3)
			RandomOptions.AddElement(MASTERY_BONUS_CURE_HARM_3, 10); //prejudica cura

		if (lpItem->m_SocketOption[4] != MASTERY_BONUS_STERN_RATE_3)
			RandomOptions.AddElement(MASTERY_BONUS_STERN_RATE_3, 10); //stern

		if (lpItem->m_SocketOption[4] != MASTERY_BONUS_BLEEDING_3)
			RandomOptions.AddElement(MASTERY_BONUS_BLEEDING_3, 10); //sangramento

		if (lpItem->m_SocketOption[4] != MASTERY_BONUS_DOUBLE_DAMAGE_BOSS_WEAPON_3)
			RandomOptions.AddElement(MASTERY_BONUS_DOUBLE_DAMAGE_BOSS_WEAPON_3, 10); //duplo boss

		if (lpItem->m_SocketOption[4] != MASTERY_BONUS_FIREING_DAMAGE)
			RandomOptions.AddElement(MASTERY_BONUS_FIREING_DAMAGE, 5); //incendiar oponente
...
bool CMysteriousStone::ApplyOnSilverHeartShield(LPOBJ lpObj, int SourceSlot, int TargetSlot) // OK
{
	if (GetLargeRand() % 100 > gServerInfo.m_MysteriouStoneSilverHeartWeaponApplyRate)
	{
		gNotice.GCNoticeSend(lpObj->Index, 3, 0, 0, 0, 0, 0, "A aplicacao de maestria falhou.");
	...
	for (int n = 0; n < 3; n++)
	{
		...
		else if (lpItem->m_SocketOption[n] >= 60 && lpItem->m_SocketOption[n] <= 65) //já tem opcional blue
		{
			lpItem->m_SocketOption[n] += 20;
	...
	else if ((lpItem->m_SocketOption[4] >= 14 && lpItem->m_SocketOption[4] <= 16)) //tem option blue
	{
		lpItem->m_SocketOption[4] += 3; //evolui pro add do silver
	}
	else
	{
		CRandomManager RandomOptions;

		if (lpItem->m_SocketOption[4] != 17)
			RandomOptions.AddElement(17, 20); //Protecao contra Dano Direto no HP

		if (lpItem->m_SocketOption[4] != 18)
			RandomOptions.AddElement(18, 20); //Aumento no Dano Refletido

		if (lpItem->m_SocketOption[4] != 19)
			RandomOptions.AddElement(19, 20); //Reducao de poderes de BOSS
