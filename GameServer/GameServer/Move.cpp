...
void CMove::Move(LPOBJ lpObj,int index) // OK
{
	...
	if((gGate.GetGateMap(MoveInfo.Gate) == MAP_ICARUS || gGate.GetGateMap(MoveInfo.Gate) == MAP_KANTURU3) && (lpObj->Inventory[7].IsItem() == 0 && lpObj->Inventory[lpObj->m_btInvenPetPos].m_Index != GET_ITEM(13,3) && lpObj->Inventory[lpObj->m_btInvenPetPos].m_Index!= GET_ITEM(13,37)))
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(228));
		return;
	}

	if (gGate.GetGateMap(MoveInfo.Gate) == MAP_TEMPORADA) //Mapa da Temporada, moves seguidos
	{
		if (GetTickCount64() - lpObj->LastPartyTeleportTime < gServerInfo.m_PartyTeleportDelay)
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Voce ainda nao pode teleportar novamente.");
			return;
		}
		else
		{
